...
const TECH_ICONS = {
  ...
  Python: Code, Javascript: Code, HTML: Code, CSS: Code, default: Package,
};
  ...
  return (
    <div className="group relative overflow-hidden px-3 py-2 md:px-4 md:py-2.5 bg-gradient-to-r from-blue-600/10 to-purple-600/10 rounded-xl border border-blue-500/10 hover:border-blue-500/30 transition-all duration-300 cursor-default">
      <div className="relative flex items-center gap-1.5 md:gap-2">
        <Icon className="w-3.5 h-3.5 md:w-4 md:h-4 text-blue-400" />
        <span className="text-xs md:text-sm font-medium text-blue-300/90">{tech}</span>
      </div>
    </div>
  );
};
...
const FeatureItem = ({ feature }) => (
  <li className="group flex items-start space-x-3 p-2.5 md:p-3.5 rounded-xl hover:bg-white/5 transition-all duration-300 border border-transparent hover:border-white/10">
    <div className="relative mt-2">
      <div className="relative w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-gradient-to-r from-blue-400 to-purple-400" />
    </div>
...
const ProjectStats = ({ project }) => {
  ...
  return (
    <div className="grid grid-cols-2 gap-3 md:gap-4 p-3 md:p-4 bg-[#0a0a1a] rounded-xl overflow-hidden relative">
      <div className="relative z-10 flex items-center space-x-2 md:space-x-3 bg-white/5 p-2 md:p-3 rounded-lg border border-blue-500/20 hover:scale-105 transition-all duration-300">
      ...
      <div className="relative z-10 flex items-center space-x-2 md:space-x-3 bg-white/5 p-2 md:p-3 rounded-lg border border-purple-500/20 hover:scale-105 transition-all duration-300">
...
const ProjectDetails = () => {
  ...
  useEffect(() => {
    ...
    const fetchProject = async () => {
      ...
      if (data) {
        const found = data.find((p) => toSlug(p.title) === slug);
  ...
  const projectUrl = `https://yabdelsa.vercel.app/project/${toSlug(project.title)}`;

  return (
    <>
      <Helmet>
        <title>{project.title} — Yahia Abdelsalam</title>
        <meta name="description" content={project.description?.slice(0, 155)} />
        <meta name="robots" content="index, follow" />
        <link rel="canonical" href={projectUrl} />
        <meta property="og:title" content={`${project.title} — Yahia Abdelsalam`} />
                ...
                <div className="flex flex-wrap gap-3 md:gap-4">
                  ...
                  {project.github && (
                    <a href={project.github} target="_blank" rel="noopener noreferrer"
                      className="group relative inline-flex items-center space-x-1.5 md:space-x-2 px-4 md:px-8 py-2.5 md:py-4 bg-gradient-to-r from-purple-600/10 to-pink-600/10 hover:from-purple-600/20 hover:to-pink-600/20 text-purple-300 rounded-xl transition-all duration-300 border border-purple-500/20 hover:border-purple-500/40 text-sm md:text-base">
                ...
                <div className="space-y-4 md:space-y-6">
                  ...
                  {project.tech_stack?.length > 0 ? (
                    <div className="flex flex-wrap gap-2 md:gap-3">
                      {project.tech_stack.map((tech, index) => <TechBadge key={index} tech={tech} />)}
                    </div>
                  ) : (
                    <p className="text-sm text-gray-400 opacity-50">No technologies added.</p>
                  )}
                </div>
              </div>
